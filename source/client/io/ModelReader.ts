 ...
 * Transient HTTP/2 stream refusals ("Failed to fetch" / `REFUSED_STREAM`) are
 * explicitly retryable per RFC 7540: a server applying back-pressure (e.g. nginx
 * `keepalive_requests`) resets the stream and expects the client to reissue the
 * request on a fresh connection. Without retrying, the largest LOD scenes drop
 * models loudly until the user changes viewpoint.
 */
const MODEL_FETCH_ATTEMPTS = 3;

/**
 * Fetch a model's bytes, retrying transient network failures with exponential backoff.
 * Aborts and client (4xx) errors are never retried. The body read is inside the
 * retry because a refused HTTP/2 stream often fails mid-stream, not at the headers.
 */
