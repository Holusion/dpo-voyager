...
export default class FontReader
{
    ...
    async load(url: string): Promise<IBitmapFont>
    {
        ...
        try {
            ...
            // itemStart is balanced here on every path: itemError alone does
            // not count the item off, so failing to load a font used to leave
            // the loading manager busy for good.
