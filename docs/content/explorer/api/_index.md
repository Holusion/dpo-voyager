...
| scene-loaded			| Fires once the scene reaches a *usable* state: every model has either an active derivative (at least Thumb quality), no Web3D derivative to load, autoLoad disabled, or has failed. Mirrors the visibility of the loading spinner (received == spinner hidden). Subsequent LOD upgrades do **not** retrigger this event - it represents "ready to interact", not "highest quality reached". No event detail; compose with `assets-idle` if you also need "no downloads in flight", and inspect `CVScene.models[i].outs.failed` if you need per-model error state.|
| scene-loading			| Fires on the reverse transition, when the scene leaves the loaded state (new document loaded, model appended via the API, scene reset). Pair with `scene-loaded` to manage external UI enable/disable. No event detail.|
| assets-loading		| Fires when the asset manager starts fetching one or more assets (textures, glTF files, overlays, JSON, ...) after having been idle. Use to show secondary loading indicators for resources beyond the main models. No event detail.|
| assets-idle			| Fires when the asset manager has nothing in flight anymore. Pairs with `assets-loading`. No event detail.|